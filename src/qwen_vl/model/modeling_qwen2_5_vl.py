...
class Qwen2_5_VLForConditionalGenerationWithGenerative(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    """Qwen2.5-VL with a pluggable generative vision encoder fused via token-gated fusion.

    Combines the original Qwen2.5-VL visual encoder (2D) with a generative
    encoder (e.g. SD2.1, WAN-VACE, SVD, V-JEPA, DINOv3, VGGT-online, VAE).
    The two feature streams are merged with token-level gated fusion before
    being fed to the LLM.

    Token alignment
    ~~~~~~~~~~~~~~~
    Qwen2.5-VL produces a **dynamic** number of visual tokens per image
    (determined by the image resolution).  The generative tower, by contrast,
    outputs a fixed spatial map.  We bridge this gap by interpolating the
    generative features to match each image's ``grid_thw``-derived patch grid
    before the spatial merger, so the final token counts are identical.
    """
    ...
    def __init__(self, config):
        ...
        if getattr(config, "use_generative_encoder", False):
            self._init_generative_encoder(config)
    ...
    # ------------------------------------------------------------------
    # Generative encoder initialisation
    # ------------------------------------------------------------------
    def _init_generative_encoder(self, config):
        """Build the generative encoder, spatial merger, and token-gated fusion."""
        from .generative_encoders import GenerativeVisionEncoder
        from .generative_encoders.encoder import GenerativeEncoderConfig

        tower_type = getattr(config, "generative_encoder_tower_type", "wan_vace_online")
        checkpoint = getattr(config, "generative_encoder_path", None)

        # Collect extra kwargs forwarded to the upstream tower constructor
        tower_kwargs = {}
        for attr in dir(config):
            if attr.startswith("generative_vision_tower_"):
                tower_kwargs[attr] = getattr(config, attr)

        gen_config = GenerativeEncoderConfig(
            tower_type=tower_type,
            checkpoint=checkpoint,
            freeze=getattr(config, "generative_encoder_freeze", True),
            tower_kwargs=tower_kwargs,
        )
        self.generative_encoder = GenerativeVisionEncoder(gen_config)

        # Merger: spatial-merge + project generative features into hidden_size
        self.generative_merger = GeometryFeatureMerger(
            output_dim=config.hidden_size,
            hidden_dim=getattr(config, "generative_merger_hidden_dim", 4096),
            context_dim=self.generative_encoder.get_feature_dim(),
            spatial_merge_size=config.vision_config.spatial_merge_size,
            merger_type=getattr(config, "generative_merger_type", "mlp"),
        )

        # Fusion (open-source release default: token_gated_residual)
        fusion_config = FeatureFusionConfig(
            fusion_method=getattr(config, "generative_fusion_method", "token_gated_residual"),
            hidden_size=config.hidden_size,
            num_heads=getattr(config, "generative_fusion_attention_heads", 8),
            dropout=getattr(config, "generative_fusion_dropout", 0.1),
            num_layers=getattr(config, "generative_fusion_num_layers", 1),
        )
        self.generative_fusion = FeatureFusionModule(fusion_config)

    # ------------------------------------------------------------------
    # Feature processing  (dynamic token alignment via image_grid_thw)
    # ------------------------------------------------------------------
    def _process_generative_features(self, image_embeds, generative_encoder_inputs, image_grid_thw):
        """Fuse Qwen 2D image embeddings with generative encoder features.

        Follows the same batched pattern as ``_process_geometry_features``:
        encode all images at once, run merger once, then fuse in a single call.

        Args:
            image_embeds: Flat ``[total_patches, hidden_size]`` from the Qwen
                visual encoder (after its internal patch-merger).
            generative_encoder_inputs: List (len = batch_size) of
                ``[n_image, 3, H, W]`` tensors (raw images).
            image_grid_thw: ``[total_images, 3]`` tensor – each row is
                ``(t, h_patches, w_patches)``.
        Returns:
            Fused embeddings in the same flat shape as *image_embeds*.
        """
        spatial_merge_size = self.config.vision_config.spatial_merge_size
        batch_size = len(generative_encoder_inputs)
        grid_offset = 0
        gen_embeds = []

        for bn in range(batch_size):
            imgs = generative_encoder_inputs[bn]  # [n_image, 3, H, W]
            n_image = imgs.shape[0]
            if n_image == 0:
                continue

            grids = image_grid_thw[grid_offset: grid_offset + n_image]
            grid_offset += n_image

            # Encode → [n_image, h_patches * w_patches, C]
            features = self.generative_encoder.encode(
                imgs, grid_thw=grids,
            ).to(image_embeds.dtype)

            # All images in the same sample share the same spatial size
            # (enforced by torch.stack in the collator).
            _, h_p, w_p = grids[0].tolist()
            h_p, w_p = int(h_p), int(w_p)
            features = features[:, : h_p * w_p, :]
            features = features.reshape(n_image, h_p, w_p, -1)

            # Batched merger → [n_image, h_m, w_m, hidden]
            features = self.generative_merger(features)
            gen_embeds.append(features)

        gen_embeds = torch.cat(gen_embeds, dim=0) if gen_embeds else None

        if gen_embeds is not None:
            image_embeds = image_embeds.view(gen_embeds.shape)
            image_embeds = self.generative_fusion(image_embeds, gen_embeds)
            image_embeds = image_embeds.view(-1, image_embeds.shape[-1])
    ...
    @classmethod
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        generative_encoder_path = kwargs.pop("generative_encoder_path", None)
        model = super().from_pretrained(pretrained_model_name_or_path, *model_args, **kwargs)
        if generative_encoder_path and hasattr(model, "generative_encoder"):
            model.generative_encoder.load_model(generative_encoder_path)
        return model

    # ------------------------------------------------------------------
    # Boilerplate accessors (identical to WithVGGT)
    # ------------------------------------------------------------------
    def get_input_embeddings(self):
        return self.model.embed_tokens
    ...
    # ------------------------------------------------------------------
    # RoPE index (reuse logic from WithVGGT)
    # ------------------------------------------------------------------
    def get_rope_index(
        self,
        input_ids: Optional[torch.LongTensor] = None,
    ...
    def forward(
        ...
        **kwargs,
        ...
        Example: see ``Qwen2_5_VLForConditionalGenerationWithVGGT``.
        """
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                # Fuse with generative encoder features
                if getattr(self.config, "use_generative_encoder", False) and geometry_encoder_inputs is not None:
                    image_embeds = self._process_generative_features(
    ...
    # ------------------------------------------------------------------
    # Generation helpers (identical to WithVGGT)
    # ------------------------------------------------------------------
    def prepare_inputs_for_generation(
        self,
        input_ids,
