...
class Seva(nn.Module):
    ...
    def forward(
        ...
        for module in self.input_blocks:
            ...
            hs.append(h)
        # Keep feature on the current device for online extraction to avoid host transfer overhead.
        self.last_input_feat = h.detach()
        ...
        #     hs_aligned.append(feat)

        # self.last_input_feat = torch.cat(hs_aligned, dim=1).detach().cpu()
        h = self.middle_block(
            h,
            emb=t_emb,
            context=y,
