...
if str(project_root) not in sys.path:
    # Ensure this repository's `src` package is resolved before any installed package.
...
def _disable_model_parallel_plans(config):
...
def set_model(model_args, model):
    ...
    if model_args.use_generative_encoder:
        # generative encoder is frozen
        for n, p in model.generative_encoder.named_parameters():
            p.requires_grad = False
...
def train(attn_implementation="flash_attention_2"):
    ...
    if "qwen2.5" in model_args.model_name_or_path.lower():
        if not model_args.use_geometry_encoder and not model_args.use_generative_encoder:
            from transformers import Qwen2_5_VLForConditionalGeneration
            config = AutoConfig.from_pretrained(model_args.model_name_or_path)
            _disable_model_parallel_plans(config)
        ...
        elif model_args.use_generative_encoder:
            from qwen_vl.model.modeling_qwen2_5_vl import Qwen2_5_VLForConditionalGenerationWithGenerative
            config = AutoConfig.from_pretrained(model_args.model_name_or_path)
            _disable_model_parallel_plans(config)

            for k in [
                "use_generative_encoder",
                "generative_encoder_tower_type",
                "generative_encoder_path",
                "generative_vision_tower_task",
                "generative_vision_tower_model_name",
                "generative_encoder_feature_dim",
                "generative_encoder_freeze",
                "generative_fusion_method",
                "generative_fusion_num_layers",
                "generative_merger_type",
            ]:
                value = getattr(model_args, k)
                if k in {"generative_vision_tower_model_name", "generative_vision_tower_task"}:
                    if value is None:
                        continue
            ...
            model = Qwen2_5_VLForConditionalGenerationWithGenerative.from_pretrained(
                ...
                attn_implementation=attn_implementation,
                torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
                generative_encoder_path=model_args.generative_encoder_path,
            )
        else:
    ...
    if model_args.use_generative_encoder:
        setattr(data_args, "use_generative_encoder", model_args.use_generative_encoder)
