...
class LazySupervisedDataset(Dataset):
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in self.list_data_dict[i]:
            ...
            if getattr(self.data_args, "use_geometry_encoder", False) or getattr(self.data_args, "use_generative_encoder", False):
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if "geometry_encoder_inputs" in instances[0]:
            geometry_encoder_inputs = [torch.stack(instance["geometry_encoder_inputs"]) for instance in instances if "geometry_encoder_inputs" in instance]
            batch["geometry_encoder_inputs"] = geometry_encoder_inputs
            tags = set(instance.get("tag", "2d") for instance in instances)
            assert len(tags) == 1, "all data in a batch should have the same tag"
