...
from llava.eval.generative_eval_utils import (
    add_generative_eval_args,
    build_model_overwrite_config,
    move_video_dict_to_device,
    resolve_generative_feature_source,
    str2bool,
)
...
def eval_model(questions, args):
    ...
    config = build_model_overwrite_config(args)
    ...
    generative_feature_source = resolve_generative_feature_source(model.config, args)
    ...
    for line in tqdm(questions):
        ...
        if line["box_input"] is not None:
            ...
            video_dict = video_processor.process_3d_video(
                ...
                generative_model_id=args.generative_model_id,
                generative_feature_source=generative_feature_source,
            )
            video_dict["box_input"] = box_input
            video_dict = merge_video_dict([video_dict])
            image_tensors, video_dict = move_video_dict_to_device(
                video_dict,
                device=model.device,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--generative_model_id", type=str, default="seva")
    parser.add_argument("--force_sample", type=str2bool, default=True)
    parser.add_argument("--overwrite_cfg", type=str2bool, default=False)
    parser.add_argument("-n", type=int, default=-1)
    parser.add_argument("--lora-path", type=str, default=None)
    add_generative_eval_args(parser)
