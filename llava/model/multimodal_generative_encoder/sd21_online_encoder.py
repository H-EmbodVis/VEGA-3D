...
class SD21OnlineEncoder(nn.Module):
    """
    Online SD2.1 generative encoder.
    - preprocess to 896x896 (default) with center-crop
    - VAE encode -> add noise at target diffusion timestep
    - one UNet forward at that exact timestep
    ...
    def __init__(self, config):
        ...
        # Backward-compat field. WAN-aligned extraction now always runs one UNet forward
        # at the selected target timestep.
    ...
    def _ensure_scheduler_ready(self, device: torch.device):
        ...
        # Use full train-time schedule so `generative_vision_tower_timestep`
    ...
    def _forward_single_video(self, frames: torch.Tensor, device: torch.device) -> torch.Tensor:
        ...
        self._ensure_scheduler_ready(device=device)
        ...
        with torch.inference_mode():
            for i in range(0, x.shape[0], self.chunk_size):
                ...
                t_start_b = tau.expand(batch_size).to(device=device)
                noise = torch.randn_like(latents)
                sample = self.scheduler.add_noise(latents, noise, t_start_b).to(device=device, dtype=self.param_dtype)

                out = self.unet(sample, t_start_b.to(dtype=torch.long), encoder_hidden_states=prompt_embeds)
