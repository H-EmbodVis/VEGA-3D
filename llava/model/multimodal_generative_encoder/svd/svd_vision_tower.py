    ...
    def ddim_one_step(self, image, pipeline, vae, unet, image_encoder,
        ...
        if all_frames_pixels is not None and all_frames_pixels.shape[1] > 0:
            ...
            if do_classifier_free_guidance:
                video_clean_latents = image_latents_with_frames[batch_size:]
            else:
                video_clean_latents = image_latents_with_frames
        ...
        # 6. Prepare timesteps.
        # WAN-aligned semantics:
        # - use full train-time schedule
        # - select exact target timestep
        # - run one UNet forward at that timestep
        if timestep is not None:
            num_train_timesteps = int(getattr(pipeline.scheduler.config, "num_train_timesteps", 1000))
            pipeline.scheduler.set_timesteps(num_train_timesteps, device=device)
            scheduler_timesteps = pipeline.scheduler.timesteps
            if scheduler_timesteps.numel() == 0:
                raise RuntimeError("SVD scheduler timesteps is empty.")
            tau_tensor = torch.tensor(int(timestep), device=scheduler_timesteps.device, dtype=scheduler_timesteps.dtype)
            tau_idx = torch.argmin(torch.abs(scheduler_timesteps - tau_tensor))
            timesteps = scheduler_timesteps[tau_idx : tau_idx + 1]
        ...
        # For WAN-aligned target-timestep extraction, prefer noisy latents from clean
        # per-frame VAE codes when available.
        if timestep is not None and video_clean_latents is not None:
            tau = timesteps[0]
            tau_batch = tau.expand(video_clean_latents.shape[0]).to(device=device)
            noise_latents = randn_tensor(
                video_clean_latents.shape,
                generator=generator,
                device=device,
                dtype=video_clean_latents.dtype,
            )
            latents = pipeline.scheduler.add_noise(video_clean_latents, noise_latents, tau_batch)
            latents = latents.to(device=device, dtype=image_embeddings.dtype)
        else:
        ...
        for i, t in enumerate(timesteps[:]):
            ...
            t = t.to(dtype=latent_model_input.dtype, device=latent_model_input.device)
