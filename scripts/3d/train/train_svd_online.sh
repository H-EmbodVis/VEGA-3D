...
SVD_MODEL_PATH="data/models/stable-video-diffusion-img2vid"
SVD_CLIP_MODEL_PATH="data/models/CLIP-ViT-H-14-laion2B-s32B-b79K"
...
RUN_NAME="${RUN_NAME:-llavanext-qwen-video3dllm-uniform-svd-online}"
...
if [ ! -d "${SVD_MODEL_PATH}" ]; then
    echo "Missing SVD checkpoint directory: ${SVD_MODEL_PATH}"
    exit 1
fi

if [ ! -e "${SVD_CLIP_MODEL_PATH}" ]; then
    echo "Missing CLIP checkpoint path: ${SVD_CLIP_MODEL_PATH}"
    exit 1
fi

torchrun --nnodes=1 --nproc_per_node="${NUM_GPUS}" --master_port 43006 \
    ...
    --generative_vision_tower_type svd_online \
    --generative_vision_tower_checkpoint $SVD_MODEL_PATH \
    --generative_vision_tower_clip_checkpoint $SVD_CLIP_MODEL_PATH \
    --generative_vision_tower_timestep 250 \
    --generative_vision_tower_height 896 \
    --generative_vision_tower_width 896 \
    --generative_vision_tower_output_spatial 14 \
    --generative_vision_tower_chunk_size 8 \
