...
VGGT_CKPT_DIR="data/models/VGGT-1B"
...
RUN_NAME="${RUN_NAME:-llavanext-qwen-video3dllm-uniform-vggt-online}"
...
if [ ! -f "${VGGT_CKPT_DIR}/model.pt" ]; then
    echo "Missing VGGT checkpoint: ${VGGT_CKPT_DIR}/model.pt"
    exit 1
fi

torchrun --nnodes=1 --nproc_per_node="${NUM_GPUS}" --master_port 43010 \
    ...
    --generative_feat_dim 2048 \
    --generative_projector_type mlp2x_gelu \
    --generative_feature_source online \
    --generative_vision_tower_type vggt_online \
    --generative_vision_tower_checkpoint $VGGT_CKPT_DIR \
    --generative_vision_tower_pretrained False \
    --generative_vision_tower_input_size 196 \
    --generative_vision_tower_output_spatial 14 \
    --generative_vision_tower_chunk_size 32 \
    --generative_vision_tower_feat_block_idx -1 \
