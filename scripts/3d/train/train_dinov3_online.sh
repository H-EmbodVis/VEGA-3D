...
DINOV3_CKPT_PATH="data/models/vit_large_patch16_dinov3.sat493m"
...
RUN_NAME="${RUN_NAME:-llavanext-qwen-video3dllm-uniform-dinov3-online}"
...
if [ ! -e "${DINOV3_CKPT_PATH}" ]; then
    echo "Missing DINOv3 checkpoint: ${DINOV3_CKPT_PATH}"
    exit 1
fi

torchrun --nnodes=1 --nproc_per_node="${NUM_GPUS}" --master_port 43009 \
    ...
    --generative_feat_dim 1024 \
    --generative_projector_type mlp2x_gelu \
    --generative_feature_source online \
    --generative_vision_tower_type dinov3_online \
    --generative_vision_tower_checkpoint $DINOV3_CKPT_PATH \
    --generative_vision_tower_model_name vit_large_patch16_dinov3 \
    --generative_vision_tower_pretrained False \
    --generative_vision_tower_input_size 224 \
