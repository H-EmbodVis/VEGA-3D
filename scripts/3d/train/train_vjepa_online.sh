...
JEPA_CKPT_PATH="data/models/vjepa2-vitg-fpc64-384-ssv2"
...
RUN_NAME="${RUN_NAME:-llavanext-qwen-video3dllm-uniform-vjepa-online}"
...
if [ ! -e "${JEPA_CKPT_PATH}" ]; then
    echo "Missing V-JEPA checkpoint: ${JEPA_CKPT_PATH}"
    exit 1
fi

torchrun --nnodes=1 --nproc_per_node="${NUM_GPUS}" --master_port 43008 \
    ...
    --generative_feat_dim 1408 \
    --generative_projector_type mlp2x_gelu \
    --generative_feature_source online \
    --generative_vision_tower_type jepa_online \
    --generative_vision_tower_checkpoint $JEPA_CKPT_PATH \
    --generative_vision_tower_model_name vit_giant_384 \
    --generative_vision_tower_img_size 224 \
    --generative_vision_tower_tubelet_size 2 \
    --generative_vision_tower_num_frames 32 \
    --generative_vision_tower_pretrained False \
