...
WAN_VACE_CKPT_DIR="data/models/Wan2.1-VACE-1.3B"
...
RUN_NAME="${RUN_NAME:-llavanext-qwen-video3dllm-uniform-wan-vace-online}"
...
if [ ! -d "${WAN_VACE_CKPT_DIR}" ]; then
    echo "Missing WAN VACE checkpoint directory: ${WAN_VACE_CKPT_DIR}"
    exit 1
fi

if [ ! -f "${WAN_PROMPT_EMBED_PATH}" ]; then
    echo "Missing WAN prompt embedding: ${WAN_PROMPT_EMBED_PATH}"
    echo "Run: python3 scripts/3d/preprocessing/export_wan_prompt_embedding.py --task vace-1.3B --checkpoint_dir ${WAN_VACE_CKPT_DIR} --output ${WAN_PROMPT_EMBED_PATH}"
    exit 1
fi

torchrun --nnodes=1 --nproc_per_node="${NUM_GPUS}" --master_port 43003 \
    ...
    --generative_vision_tower_type wan_vace_online \
    --generative_vision_tower_checkpoint $WAN_VACE_CKPT_DIR \
    --generative_vision_tower_task vace-1.3B \
    --generative_vision_tower_size 832*480 \
    --generative_vision_tower_timestep 300 \
    --generative_vision_tower_shift 5.0 \
    --generative_vision_tower_context_scale 1.0 \
    --generative_vision_tower_feat_block_idx 20 \
    --generative_vision_tower_condition_on_first_frame True \
