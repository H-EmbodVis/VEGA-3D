...
RUN_NAME="${RUN_NAME:-llavanext-qwen-video3dllm-uniform-vae-online}"
...
if [ ! -d "${SD21_BASE_CKPT_DIR}/vae" ]; then
    echo "Missing SD2.1 VAE directory: ${SD21_BASE_CKPT_DIR}/vae"
    exit 1
fi

torchrun --nnodes=1 --nproc_per_node="${NUM_GPUS}" --master_port 43007 \
